...
if platform.system().lower() == 'windows':
    ...
    bass_mpc_module = ctypes.CDLL('libbass_mpc.dylib', mode=ctypes.RTLD_GLOBAL)
    func_type = ctypes.CFUNCTYPE
else:
    bass_mpc_module = ctypes.CDLL('libbass_mpc.so', mode=ctypes.RTLD_GLOBAL)
    func_type = ctypes.CFUNCTYPE
...
BASS_MPC_StreamCreateFile = func_type(HSTREAM, ctypes.c_byte, ctypes.c_void_p, QWORD, QWORD, ctypes.c_ulong)(
...
BASS_MPC_StreamCreateURL = func_type(HSTREAM, ctypes.c_char_p, ctypes.c_ulong, ctypes.c_ulong, DOWNLOADPROC,
                                     ctypes.c_void_p)(('BASS_MPC_StreamCreateURL', bass_mpc_module))
# HSTREAM BASSMPCDEF(BASS_MPC_StreamCreateFileUser)(DWORD system, DWORD flags, const BASS_FILEPROCS *proc, void *user);
BASS_MPC_StreamCreateFileUser = func_type(HSTREAM, ctypes.c_ulong, ctypes.c_ulong, ctypes.POINTER(BASS_FILEPROCS),
                                          ctypes.c_void_p)(('BASS_MPC_StreamCreateFileUser', bass_mpc_module))
