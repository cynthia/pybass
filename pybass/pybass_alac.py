...
if platform.system().lower() == 'windows':
    ...
    raise RuntimeError("ALAC is not supported on macOS")
else:
    bass_alac_module = ctypes.CDLL('libbassalac.so', mode=ctypes.RTLD_GLOBAL)
...
BASS_ALAC_StreamCreateFile = func_type(HSTREAM, ctypes.c_byte, ctypes.c_void_p, QWORD, QWORD, ctypes.c_ulong)(
...
BASS_ALAC_StreamCreateURL = func_type(HSTREAM, ctypes.c_char_p, ctypes.c_ulong, ctypes.c_ulong, DOWNLOADPROC,
                                      ctypes.c_void_p)(('BASS_ALAC_StreamCreateURL', bass_alac_module))
# HSTREAM BASSALACDEF(BASS_ALAC_StreamCreateFileUser)(DWORD system, DWORD flags, const BASS_FILEPROCS *procs, void *user);
BASS_ALAC_StreamCreateFileUser = func_type(HSTREAM, ctypes.c_ulong, ctypes.c_ulong, ctypes.POINTER(BASS_FILEPROCS),
