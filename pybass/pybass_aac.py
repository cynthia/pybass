...
if platform.system().lower() == 'windows':
    ...
    func_type = None
    # raise RuntimeError("AAC is not supported on Darwin")
else:
    bass_aac_module = ctypes.CDLL('libbass_aac.so', mode=ctypes.RTLD_GLOBAL)
...
if func_type:
    ...
    BASS_AAC_StreamCreateFile = func_type(HSTREAM, ctypes.c_byte, ctypes.c_void_p, QWORD, QWORD, ctypes.c_ulong)(
    ...
    BASS_AAC_StreamCreateURL = func_type(HSTREAM, ctypes.c_char_p, ctypes.c_ulong, ctypes.c_ulong, DOWNLOADPROC,
                                         ctypes.c_void_p)(('BASS_AAC_StreamCreateURL', bass_aac_module))
    # HSTREAM BASSAACDEF(BASS_AAC_StreamCreateFileUser)(DWORD system, DWORD flags, const BASS_FILEPROCS *procs, void *user);
    BASS_AAC_StreamCreateFileUser = func_type(HSTREAM, ctypes.c_ulong, ctypes.c_ulong, ctypes.POINTER(BASS_FILEPROCS),
                                              ctypes.c_void_p)(('BASS_AAC_StreamCreateFileUser', bass_aac_module))
    # HSTREAM BASSAACDEF(BASS_MP4_StreamCreateFile)(BOOL mem, const void *file, QWORD offset, QWORD length, DWORD flags);
    BASS_MP4_StreamCreateFile = func_type(HSTREAM, ctypes.c_byte, ctypes.c_void_p, QWORD, QWORD, ctypes.c_ulong)(
    ...
    BASS_MP4_StreamCreateFileUser = func_type(HSTREAM, ctypes.c_ulong, ctypes.c_ulong, ctypes.POINTER(BASS_FILEPROCS),
                                              ctypes.c_void_p)(('BASS_MP4_StreamCreateFileUser', bass_aac_module))
