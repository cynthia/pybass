...
class player_ctrl(wx.Panel):
    def __init__(self, *args, **kwargs):
        ...
        self.status_line = Ticker(self, fgcolor='#000062', bgcolor='#7F7F8F', start=False, ppf=1, fps=50,
                                  direction='ltr')
    ...
    def method_load_file(self):
        ...
        for plugin in self.plugins.values():
            if plugin[0] > 0:
                wildcard += plugin[1]
        wildcard += '|All files (*.*)|*.*'
        dlg = wx.FileDialog(self, message=_('Choose a file'), defaultDir=os.getcwd(), defaultFile='', wildcard=wildcard,
    ...
    def method_load_data(self, stream, name_stream='memory_stream'):
        if stream is not None:
            ...
            if isinstance(self.stream, memory_stream):
                ...
                new_bass_handle = pybass.BASS_StreamCreateFileUser(system, flags, self.bass_file_procs,
                                                                   id(self.stream.data))
                if new_bass_handle == 0:
                    print(('BASS_StreamCreateFileUser error %s' % pybass.get_error_description(
...
if __name__ == "__main__":
    ...
    class main_frame(wx.Frame):
        def __init__(self, *args, **kwargs):
            ...
            self.aui_manager.AddPane(self.log_ctrl,
                                     AuiPaneInfo().Name('log_ctrl').Bottom().Layer(0).BestSize((100, 100)).Hide())
            ...
            if os.path.exists('lang/%s' % lang_catalog):
                for dir_name in os.listdir('lang'):
                    if os.path.exists('lang/%s/%s.mo' % (dir_name, self.app_name)):
                        ...
                        list_trans.append(
                            gettext.GNUTranslations(open('lang/%s/%s.mo' % (dir_name, self.app_name), 'rb')))
                        i += 1
                if len(list_trans) > 0:
                    try:
                        list_trans[current_trans].install(str=True)  # wx.USE_UNICODE
                    except:
                        print_error()
            if current_trans == -1:
                trans = gettext.NullTranslations()
                trans.install(str=True)  # wx.USE_UNICODE
