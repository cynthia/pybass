...
class OggPage(object):
    ...
    def pack(self):
        header = struct.pack("<4sBBqLL", b'OggS', self.version, self.headerType, self.granulePosition, self.streamID,
                             self.sequenceNumber)
    ...
    def __repr__(self):
        ...
        self.version, self.headerType, self.streamID, self.granulePosition, self.sequenceNumber, self.continued,
        len(self.segments), self.payload())
...
class OggReader(Parser):
    ...
    def processHeader(self, data):
        ...
        (magic, p.version, p.headerType, p.granulePosition, p.streamID, p.sequenceNumber, p.checksum,
         nseg) = struct.unpack("<4sBBqLLLB", data)
        if magic != b'OggS':
...
class VorbisIdentification(object):
    ...
    def __repr__(self):
        ...
        self.version, self.audioChannels, self.sampleRate, self.minimumBitrate, self.nominalBitrate,
        self.maximumBitrate, self.blocksize)

    def positionToSeconds(self, position):
        return float(position) / self.sampleRate

    def pack(self):
        return ''.join([self.PACKET_HEADER,
                        s.pack("<IBIiiiBB", self.version, self.audioChannels, self.sampleRate, self.maximumBitrate,
...
class VorbisStreamReader(OggStreamReader):
    ...
    def read_identification(self, packet):
        ...
        (i.version, i.audioChannels, i.sampleRate, i.maximumBitrate, i.nominalBitrate, i.minimumBitrate, i.blocksize,
         framingFlag) = s.unpack("<IBIiiiBB")
        assert len(s) == 0
